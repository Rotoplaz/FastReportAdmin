import { OverviewData, OverViewMode, Report } from "../interfaces/reports.interfaces"
...
export const filterOverViewData = (
  ...
  if (groupBy === "day") {
    ...
    while (tmp <= now) {
      ...
      const name = tmp.toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
      }) 
    ...
    const tmp = new Date(startDate)
    while (tmp <= now) {
      const key = tmp.toLocaleString("default", { month: "short" }).toLowerCase()
